...
const PROVINCE = "Islamabad Capital Territory";
...
async function run() {
  const dataset = JSON.parse(
    await readFile(
      path.join(process.cwd(), "data", "imports", "islamabad-pdf-hospitals-2026.json"),
      "utf8",
    ),
  ) as PdfDataset;
  ...
  for (const hospital of dataset.hospitals) {
    ...
    const { data: insertedRow, error } = await supabase
      ...
      .insert({
        ...
        source_id: sourceId,
        name: hospital.name,
        slug: `pdf-ict-${slugify(hospital.name)}`,
    ...
    const { error: eventError } = await supabase.from("verification_events").insert({
      ...
      evidence: {
        importBatch: "islamabad-pdf-hospitals-2026",
        sourceName: dataset.sourceName,
        notes: "Imported from the provided Islamabad hospital PDF as instructed.",
